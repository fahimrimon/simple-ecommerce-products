...
const Product = ({product}) => {
  ...
  return <div>
    <div className='border border-[#e4e4e4] h-[300px] mb-4 relative overflow-hidden group transition'>
      ...
      <div className='absolute top-6 -right-11 group-hover:right-3 p-2 flex flex-col items-center justify-center gap-y-2 opacity-0 group-hover:opacity-100 transition-all duration-300'>
        <button onClick={() => addToCart(product, id)}>
            <div className='flex justify-center items-center text-white bg-red-500 h-8 w-8'>
              <BsPlus className='text-3xl'/>
              </div>
        </button>
        <Link to={`/product/${id}`} className='h-8 w-8 bg-white flex justify-center items-center text-primary drop-shadow-xl'>
          <BsEyeFill></BsEyeFill>
        </Link>
      </div>
    </div>
    ...
    <div>
      ...
      <Link to={`/product/${id}`}><h2 className='font-semibold mb-2'>{title}</h2></Link>
